...
public abstract partial class ParticleAttackVfx : Node2D
{
        ...
        float flow=.01f,Action<Sprite2D,float>? motion=null)
    {
        ...
        Vector2 scale=size/texture.GetSize()*PowerScale;
        ...
        _layers.Add((node,material,(sprite,u)=>{
            sprite.Scale=scale*(.84f+.25f*Ease(0,.7f,u));sprite.Rotation=angle;sprite.Position=position*PowerScale;
        ...
        float start=.04f,bool follow=false,float spread=180,Vector3? direction=null)
    {
        ...
        var process=new ParticleProcessMaterial{
            ...
            EmissionShape=ParticleProcessMaterial.EmissionShapeEnum.Sphere,EmissionSphereRadius=35*PowerScale,
            InitialVelocityMin=speed*.45f*PowerScale,InitialVelocityMax=speed*PowerScale,
            Gravity=new Vector3(0,tile==Smoke?-42:tile==Dust?95:15,0),
            DampingMin=30,DampingMax=65,AngularVelocityMin=-65,AngularVelocityMax=65,
            ScaleMin=particleSize/texture.GetWidth()*.6f*PowerScale,ScaleMax=particleSize/texture.GetWidth()*PowerScale,
